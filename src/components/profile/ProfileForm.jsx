...
const FloatingInputWithIcon = ({ label, type = "text", icon: Icon, value, onChange, disabled = false, error }) => {
  ...
  return (
    <div className="w-full relative">
      {hasValue && <label className="block text-gray-500 mb-1 text-sm font-medium">{label}</label>}
      <div className="relative">
        ...
        <Input
          ...
          disabled={disabled}
          className={`pl-10 pt-3 pb-2 transition-colors duration-300 ${error ? "border-red-500 focus-visible:ring-red-500" : ""} ${
...
const getOneUser = async () => {
  ...
  try {
    ...
    Swal.fire({
      ...
      text: err.response?.data?.message || "Network error. Check URL or server",
    });
  }
};
...
const handleUpdateUser = async (id, updatedData) => {
  const token = getAuthToken();
  try {
    const { data } = await axios.put(`${API_BASE_URL}user/update_user/${id}`, updatedData, {
...
const handleDeleteAccount = async (id) => {
  ...
  try {
    ...
    Swal.fire({ icon: "success", title: "Account Deleted", text: "Your account has been permanently deleted." });
  } catch (err) {
...
const ProfileForm = () => {
  ...
  return (
    ...
    <div className="flex flex-col gap-3 w-full pb-6">
      ...
      <Button className="bg-red-500 mt-2" onClick={() => handleDeleteAccount("6914960bd7fa7d80b92963e9")}>
        Delete Account
      </Button>
